...
Class pendingpayableModel Extends baseModel {
	protected $table = "pending_payable";
    ...
    public function deleteCosts($id){
    	if ($this->getCosts($id)) {
    		return $this->delete($this->table,array('pending_payable_id'=>$id));
    	}
    }
    ...
    public function getAllCostsByWhere($id){
        return $this->query('SELECT * FROM pending_payable WHERE pending_payable_id != '.$id);
    }
