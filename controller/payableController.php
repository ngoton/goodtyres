...
Class payableController Extends baseController {
        ...
        if (isset($_POST['data'])) {
            ...
            if($payable_data->vendor > 0){
                ...
                $data_owe = array(
                            ...
                            'import_tire' => $payable_data->import_tire,
                            'order_tire' => $payable_data->order_tire,
                        );
                if($data_owe['week'] == 53){
                    $data_owe['week'] = 1;
    ...
    public function add(){
        ...
        if ($_SESSION['role_logined'] != 1 && $_SESSION['role_logined'] != 2 && $_SESSION['role_logined'] != 3 && $_SESSION['role_logined'] != 7 && $_SESSION['role_logined'] != 8) {
            return $this->view->redirect('user/login');
        }
        if (isset($_POST['yes'])) {
            ...
            if ($_POST['yes'] != "") {
                ...
                if($sale_vendor->getVendorByWhere(array('trading'=>$payable_data->trading,'vendor'=>$payable_data->vendor,'type' => $payable_data->cost_type))){
                    $id_sale_vendor = $sale_vendor->getVendorByWhere(array('trading'=>$payable_data->trading,'vendor'=>$payable_data->vendor,'type' => $payable_data->cost_type));
                    $sale_vendor_data = array(
                        'comment' => $data['comment'],
                    );

                    $sale_vendor->updateVendor($sale_vendor_data,array('sale_vendor_id'=>$id_sale_vendor->sale_vendor_id));
                }
                else if($sale_vendor->getVendorByWhere(array('sale_report'=>$payable_data->sale_report,'vendor'=>$payable_data->vendor,'type' => $payable_data->cost_type))){
                    $id_sale_vendor = $sale_vendor->getVendorByWhere(array('sale_report'=>$payable_data->sale_report,'vendor'=>$payable_data->vendor,'type' => $payable_data->cost_type));
                    $sale_vendor_data = array(
                        'comment' => $data['comment'],
                    );
