...
Class transportController Extends baseController {
    public function index() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $order_by = $this->registry->router->order_by ? $this->registry->router->order_by : 'transport_id';
        ...
        $tongsodong = count($transport_model->getAllTransport());
        ...
        $this->view->data['lastID'] = isset($transport_model->getLastTransport()->transport_id)?$transport_model->getLastTransport()->transport_id:0;
        
        $this->view->show('transport/index');
    }
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            $transport = $this->model->get('transportModel');
            $data = array(
                        
                        'c20_feet' => trim(str_replace(',','',$_POST['c20_feet'])),
                        'c40_feet' => trim(str_replace(',','',$_POST['c40_feet'])),
                        'c20_ton' => trim(str_replace(',','',$_POST['c20_ton'])),
                        'c28_ton' => trim(str_replace(',','',$_POST['c28_ton'])),
                        'over_28_ton' => trim(str_replace(',','',$_POST['over_28_ton'])),
                        );
            if ($_POST['check'] == "") {
                $data['transport_update_user'] = $_SESSION['userid_logined'];
                $data['transport_update_time'] = time();
                //var_dump($data);
                $transport->updateTransport($data,array('transport_id' => $_POST['yes']));
                echo "Cập nhật thành công";
                date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."edit"."|".$_POST['yes']."|transport|".implode("-",$data)."\n"."\r\n";
                ...
                if ($transport->getTransportByWhere(array('loc_from'=>$_POST['loc_from'],'loc_to' => $_POST['loc_to']))) {
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."add"."|".$transport->getLastTransport()->transport_id."|transport|".implode("-",$data)."\n"."\r\n";
                ...
                if ($_POST['loc_from'] != 1 && $_POST['loc_to'] != 1) {
                    if ($_POST['loc_from'] !=  $_POST['loc_to']) {
                        ...
                        if ($r && $t) {
                            ...
                            $from_c20_ton = $r->c20_ton;
                            //var_dump($r->c20_feet);
                            $to_c20_feet = $t->c20_feet;
                            $to_c20_ton = $t->c20_ton;

                            $data['c20_feet'] = (($from_c20_feet + $to_c20_feet)/2)+1000000;
                            $data['c20_ton'] = (($from_c20_ton + $to_c20_ton)/2)+1000000;

                            $data['c40_feet'] = $data['c20_feet'] + 300000;
                            $data['c28_ton'] = $data['c20_ton'] + 500000;
                            $data['over_28_ton'] = $data['c28_ton'] + ($data['c20_ton']*(30/100));
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."delete"."|".$data."|transport|"."\n"."\r\n";
                ...
                date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."delete"."|".$_POST['data']."|transport|"."\n"."\r\n";
    ...
    public function import(){
                    ...
                    if ($val[0] != null && $val[1] != null) {
                        ...
                        if(!$location->getLocationByWhere(array('location_name'=>trim($val[0]),'district'=>$id_district))){
                            ...
                            $transport_data = array(
                                ...
                                'loc_to' => $id_location,
                                'c20_feet' => trim($val[3]),
                                'c40_feet' => trim($val[4])!=null?trim($val[4]):trim($val[3])+300000,
                                );
                            ...
                            if (!$transport->getTransportByWhere(array('loc_from'=> 1,'loc_to' => $id_location))) {
                                $transport_data = array(
                                'loc_from' => 1, // Cát Lái
                                'loc_to' => $id_location,
                                'c20_feet' => trim($val[3]),
                                ...
                                $id_transport = $transport->getTransportByWhere(array('loc_from'=> 1,'loc_to' => $id_location))->transport_id;
                                $transport_data = array(
                                'c20_feet' => trim($val[3]),
                                'c40_feet' => trim($val[4])!=null?trim($val[4]):trim($val[3])+300000,
                                );
                                $transport->updateTransport($transport_data,array('transport_id' => $id_transport));
                            }
                        }
                    }
                    ...
                    if ($val[1] != null && $val[2] != null) {
                        ...
                        if(!$location->getLocationByWhere(array('location_name'=>$loc_from,'district'=>$id_district_from))){
                            ...
                            $transport_data = array(
                                ...
                                'c20_feet' => trim($val[4])!=null?trim($val[4]):trim($val[5])-300000,
                                'c40_feet' => trim($val[5]),
                                );
                            $transport->createTransport($transport_data);
                        }
                            ...
                            if (!$transport->getTransportByWhere(array('loc_from'=> $id_location_from,'loc_to' => $id_location_to))) {
                                $transport_data = array(
                                    'loc_from' => $id_location_from,
                                    'loc_to' => $id_location_to,
                                    'c20_feet' => trim($val[4])!=null?trim($val[4]):trim($val[5])-300000,
                                ...
                                $id_transport = $transport->getTransportByWhere(array('loc_from'=> $id_location_from,'loc_to' => $id_location_to))->transport_id;
                                $transport_data = array(
                                'c20_feet' => trim($val[4])!=null?trim($val[4]):trim($val[5])-300000,
                                'c40_feet' => trim($val[5]),
                                );
        ...
        $this->view->show('transport/import');
