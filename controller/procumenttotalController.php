<?php
Class procumenttotalController Extends baseController {
    public function index() {
        ...
        $query = 'SELECT sale_vendor_id, vendor, type, sale_vendor.cost, sale_vendor.cost_vat, sale_vendor.document_cost, sale_vendor.pay_cost, sale_vendor.invoice_cost, sale_vendor.comment, sale_report, sale_report_id, code, sale_vendor.expect_date, bank_id, bank_name, shipment_vendor_id, shipment_vendor_name, sale_vendor.source FROM sale_vendor, sale_report, bank, shipment_vendor WHERE sale_report=sale_report_id AND vendor=shipment_vendor_id AND sale_vendor.source=bank_id AND sale_type = 1 AND sale_date >= '.strtotime($batdau).' AND sale_date <= '.strtotime($ketthuc);

        if ($keyword != "") {
            $query .= ' AND sale_report.code LIKE "%'.$keyword.'%" ';
        }

        $sale_model = $this->model->get('salevendorModel');
        $sonews = $limit;
        $x = ($page-1) * $sonews;
        $pagination_stages = 2;
        ...
        $tongsodong = count($sale_model->queryVendor($query));
        ...
        $all_sale = $sale_model->queryVendor($query);
        
        $this->view->data['sales'] = $all_sale;
        $this->view->data['lastID'] = isset($sale_model->getLastVendor()->sale_vendor_id)?$sale_model->getLastVendor()->sale_vendor_id:0;

        $cost_model = $this->model->get('costsModel');



        $sales = $sale_report_model->getAllSale($data);
        $sales_data = array();
        foreach ($sales as $sale) {
            $sales_data[$sale->code]['revenue'] = $sale->revenue_vat+$sale->revenue+$sale->payhalf;
            $sales_data[$sale->code]['cost'] = $sale->cost_vat+$sale->cost+$sale->estimate_cost+$sale->payhalf;
            $sales_data[$sale->code]['comment'] = $sale->comment;
            $data_cost = array(
            ...
            foreach ($costs as $cost) {
                if ($cost->check_invoice==1) {
                    $sales_data[$sale->code]['invoice'] = isset($sales_data[$sale->code]['invoice'])?$sales_data[$sale->code]['invoice']+$cost->pay_money:$cost->pay_money;
                }
                else if ($cost->check_document==1) {
                    $sales_data[$sale->code]['document'] = isset($sales_data[$sale->code]['document'])?$sales_data[$sale->code]['document']+$cost->pay_money:$cost->pay_money;
                }
                else if ($cost->check_pay==1) {
                    $sales_data[$sale->code]['pay'] = isset($sales_data[$sale->code]['pay'])?$sales_data[$sale->code]['pay']+$cost->pay_money:$cost->pay_money;
                }
                
            }
        }

        $this->view->data['sales_data'] = $sales_data;

        /* Lấy tổng doanh thu*/
        
        /*************/
        $this->view->show('procumenttotal/index');
    }
