...
Class ordertireController Extends baseController {
        ...
        foreach ($order_tires as $tire) {
            ...
            foreach ($sales as $sale) {
                ...
                foreach ($tire_price_discounts as $price) {
                    ...
                    $tire_price_origin = round(($price->tire_price*0.75)/1000)*1000; // giá công khai giảm 25% + vc
                    $giacongkhai = $price->tire_price; // giá công khai
                    ...
                    foreach ($tire_price_discount_events as $event) {
                        if ($event->percent_discount > 0) {
                            ...
                            $tire_price_origin = ((round(($price->tire_price*0.75)/1000)*1000))*((100-$event->percent_discount)/100);
                            ...
                            $tire_price_origin = (round(($price->tire_price*0.75)/1000)*1000)-$event->money_discount;
                ...
                if ($tire->customer_type==1) {
                    ...
                    $salary = (($tire_price_agents-$tire_price_origin)*$arr_salary['phantram']/100)*$sale->tire_number;
                    if ($dongia >= $tire_price_agents) {
                        $salary += ($dongia-$tire_price_agents)*0.5*$sale->tire_number;
                    }
                    else{
                    ...
                    $salary = (($tire_prices-$tire_price_origin)*$arr_salary['phantram']/100)*$sale->tire_number;

                    if ($dongia >= $tire_prices) {
                        $salary += ($dongia-$tire_prices)*0.5*$sale->tire_number;
                    }
                    else{
                        $salary = $arr_salary['sanluong']*$sale->tire_number;
                        // if ($sl<100) {
    ...
    public function getoldprice(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $data = array(
                'where'=>'sale='.trim($_POST['sale']).' AND tire_brand='.$brand.' AND tire_size='.$size.' AND tire_pattern='.$pattern.' AND expired_date > '.strtotime(date('d-m-Y')),
                'order_by'=>'expired_date DESC',
                'limit'=>1,
            );
            $locks = $order_tire_lock_model->getAllTire($data);
            foreach ($locks as $lock) {
                $price['lock'] = 1;
                $price['msg'] = "Bạn không thể đặt hàng này vì đang bị khóa 24h";
            }
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                        ...
                        foreach ($pa as $p) {
                            ...
                            $additionals = $additional_model->getAdditionalByWhere(array('payable'=>$p->payable_id));
    ...
    public function listtire($id){
        ...
        foreach ($order_tire_lists as $order) {
            ...
            $prices = $tire_price_discount_model->getAllTire(array('where'=>'tire_brand='.$order->tire_brand.' AND tire_size='.$order->tire_size.' AND tire_pattern='.$order->tire_pattern.' AND start_date < '.$ngaysau.' AND (end_date IS NULL OR end_date > '.$ngaytruoc.')  ORDER BY start_date DESC LIMIT 1'));
            foreach ($prices as $p) {
    ...
    public function deleteagentorder(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $order_tire_list = $order_tire_list_model->getTireByWhere(array('tire_brand'=>$brand,'tire_size'=>$size,'tire_pattern'=>$pattern,'order_tire'=>$_POST['id_order_tire']));
            ...
            $data_purchase = array(
                ...
                'purchase_tire_number'=>$_POST['total_number'],
                'purchase_tire_total'=>$_POST['total'],
                'id_order_vendor'=>$_POST['id_order_vendor'],
    ...
    public function deleteorderagentorder(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $tire_pattern_model = $this->model->get('tirepatternModel');
            $tire_brand_model = $this->model->get('tirebrandModel');
            $tire_size_model = $this->model->get('tiresizeModel');
            ...
            $purchase_tire_model->updateTire($data_purchase,array('order_tire'=>$_POST['id_order_tire']));
            $id_purchase = $purchase_tire_model->getTireByWhere(array('order_tire'=>$_POST['id_order_tire']))->purchase_tire_id;

            $purchase_tire_detail_model->queryTire('DELETE FROM purchase_tire_detail WHERE tire_brand='.$brand.' AND tire_size='.$size.' AND tire_pattern='.$pattern.' AND purchase_tire='.$id_purchase);
        }
    }
    ...
    public function editagentorder(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            foreach ($order_tire as $v) {
                ...
                if (!$purchase_details) {
                    $data_purchase_detail = array(
                        'purchase_tire'=>$purchases->purchase_tire_id,
                        'tire_brand'=>$brand,
                        'tire_size'=>$size,
                        'tire_pattern'=>$pattern,
                    ...
                    $purchase_tire_detail_model->createTire($data_purchase_detail);
                    $id_purchase_detail = $purchase_tire_detail_model->getLastTire()->purchase_tire_detail_id;
                }
                else{
                    $data_purchase_detail = array(
                        'tire_price'=>$v['tire_price'],
                    ...
                    $id_purchase_detail = $purchase_details->purchase_tire_detail_id;
                }
                $total+=$v['tire_number'];

                if ($items=="") {
                    $items = $id_purchase_detail;
                }
                else{
                    $items .= ",".$id_purchase_detail;
                }
            ...
            //delete not in item
            $purchase_tire_detail_model->queryTire('DELETE FROM purchase_tire_detail WHERE purchase_tire='.$purchases->purchase_tire_id.' AND purchase_tire_detail_id NOT IN ('.$items.')');

            $data_purchase = array(
                'purchase_tire_total'=>trim($_POST['total']),
                'purchase_tire_number'=>$total,
            );
    ...
    public function addordercost(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $tk_6418 = $account_model->getAccountByWhere(array('account_number'=>'6418'))->account_id;
            $tk_141 = $account_model->getAccountByWhere(array('account_number'=>'141_chiphi'))->account_id;
            ...
            foreach ($vendor_cost as $v) {
                ...
                if ($v['vendor']>0) {
                    ...
                    if ($order_cost_model->getTireByWhere(array('order_tire'=>$cost_data['order_tire'],'vendor'=>$cost_data['vendor'],'order_tire_cost_type'=>$cost_data['order_tire_cost_type']))) {
                        ...
                        $payables = $payable_model->getCostsByWhere(array('check_cost'=>4,'money'=>$data_order_cost->order_tire_cost,'vendor' => $cost_data['vendor'],'order_tire'=>trim($_POST['data']),'cost_type' => $cost_data['order_tire_cost_type']));
                        if($payables){
                            
                            if ($payables->money >= $payable_data['money'] && $payables->approve > 0) {
                                $payable_data['approve'] = 10;
                            }

                            $payable_model->updateCosts($payable_data,array('payable_id'=>$payables->payable_id));
                            
                            
                            $data_additional = array(
                                 
                                'document_date' => $payable_data['payable_date'],
                                'additional_date' => $payable_data['payable_date'],
                                'additional_comment' => $payable_data['comment'],
                                'debit' => $tk_6418,
                                'credit' => $tk_141,
                                'code' => $payable_data['code'],
                                'money' => $payable_data['money'],
                            ...
                            $additional_id = $additional_model->getAdditionalByWhere(array('payable'=>$payables->payable_id))->additional_id;
                            $add = $additional_model->getAdditional($additional_id);
                        ...
                        $payables = $payable_model->getLastCosts();
                        $data_additional = array(
