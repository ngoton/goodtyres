...
Class tireimportController Extends baseController {
    public function index() {
        ...
        $join = array('table'=>'tire_brand, tire_size, tire_pattern','where'=>'tire_brand.tire_brand_id = tire_import.tire_brand AND tire_size.tire_size_id = tire_import.tire_size AND tire_pattern.tire_pattern_id = tire_import.tire_pattern');

        $tire_import_model = $this->model->get('tireimportModel');

        $list_code = $tire_import_model->queryTire('SELECT code FROM tire_import GROUP BY code ORDER BY code ASC');
        ...
        $tongsodong = count($tire_import_model->getAllTire($data,$join));
        ...
        $this->view->data['tire_imports'] = $tire_import_model->getAllTire($data,$join);
        $this->view->data['lastID'] = isset($tire_import_model->getLastTire()->tire_import_id)?$tire_import_model->getLastTire()->tire_import_id:0;

        /* Lấy tổng doanh thu*/
        
        /*************/
        $this->view->show('tireimport/index');
    }
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            if ($_POST['yes'] != "") {
                    ...
                    $tire_import_model->updateTire($data,array('tire_import_id' => trim($_POST['yes'])));
                    echo "Cập nhật thành công";

                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."edit"."|".$_POST['yes']."|tire_import|".implode("-",$data)."\n"."\r\n";
                    ...
                    $tire_import_model->createTire($data);
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."add"."|".$tire_import_model->getLastTire()->tire_import_id."|tire_import|".implode("-",$data)."\n"."\r\n";
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                       $tire_import_model->deleteTire($data);
                        echo "Xóa thành công";
                        date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."delete"."|".$data."|tire_import|"."\n"."\r\n";
                ...
                return true;
                        ...
                        $tire_import_model->deleteTire($_POST['data']);
                        echo "Xóa thành công";
                        date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        $filename = "action_logs.txt";
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."delete"."|".$_POST['data']."|tire_import|"."\n"."\r\n";
    ...
    public function import(){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_FILES['import']['name'] != null) {
                ...
                for ($row = 3; $row <= $highestRow; ++ $row) {
                    ...
                    if ($val[1] != null && $val[2] != null && $val[3] != null && $val[4] != null) {
                            ...
                            if ($id_brand != null && $id_size != null && $id_pattern != null) {
                                ...
                                if($tireimport->getTireByWhere(array('tire_brand'=>$id_brand,'tire_size'=>$id_size,'tire_pattern'=>$id_pattern,'code'=>$code))) {
                                    $id_tire_import = $tireimport->getTireByWhere(array('tire_brand'=>$id_brand,'tire_size'=>$id_size,'tire_pattern'=>$id_pattern,'code'=>$code))->tire_import_id;
            ...
            return $this->view->redirect('tireimport');
        }
        $this->view->show('tireimport/import');
